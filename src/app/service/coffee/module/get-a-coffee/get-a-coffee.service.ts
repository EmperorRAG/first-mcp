 ...
 *    {@link GetACoffeeInputStandard} as `inputSchema`.
 * 2. In the async handler, decodes the raw `args` with
 *    {@link Schema.decodeUnknownSync} against
 *    {@link GetACoffeeInput} to extract the validated `name`.
 * 3. Delegates to
 *    {@link GetACoffeeService.executeFormatted | executeFormatted}
 ...
 *   includes {@link GetACoffeeService}.
 */
		...
		async (args) => {
			...
			return runtime.runPromise(
				Effect.gen(function* () {
					...
					return yield* service.executeFormatted(name);
				}),
			);
		},
	);
