/**
 * Unit tests for the standalone HTTP route resolver ({@link resolve}).
 *
 * @remarks
 * Validates the resolution logic: DNS rebinding guard, CORS preflight,
 * health check, MCP paths, and not-found fallback.
 *
 * @module
 */
import { describe, it, expect } from "vitest";
import { ConfigProvider, Effect, Layer } from "effect";
import { resolve as routeResolve } from "./resolve.js";
...
describe("resolve", () => {
	...
	it("accepts 127.0.0.1 as valid host", async () => {
		expect(
			await resolve({ host: "127.0.0.1:3001", method: "GET", path: "/health" }),
		).toBe("health-check");
	});

	it("returns forbidden for bare ::1 (simple split does not handle IPv6)", async () => {
		expect(
			await resolve({ host: "::1", method: "GET", path: "/health" }),
		).toBe("forbidden");
	});
